...
keep_dars <- !is.na(res$padj) & res$padj < 0.05

dars <- rowRanges(dds_sea)[keep_dars]

mcols(dars)$baseMean <- res$baseMean[keep_dars]
mcols(dars)$log2FC <- res$log2FoldChange[keep_dars]
mcols(dars)$lfcSE <- res$lfcSE[keep_dars]
mcols(dars)$stat <- res$stat[keep_dars]
mcols(dars)$pvalue <- res$pvalue[keep_dars]
mcols(dars)$padj <- res$padj[keep_dars]
